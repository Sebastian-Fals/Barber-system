...
class TestFixes(unittest.TestCase):
    def setUp(self):
        ...
        self.barber = (
            self.db.query(Barber).filter(Barber.name == "Juan Perez", Barber.business_id == self.business.id).first()
        )
        if not self.barber:
    ...
    def test_loose_barber_match(self, mock_llm, mock_whatsapp):
        ...
        mock_llm.analyze_message.return_value = {
            ...
            "extracted": {"barber_name": "juan", "date": "2023-12-25"},  # Lowercase, partial
            "reply": "Checking...",
        }
    ...
    def test_empathetic_date_error(self, mock_llm, mock_whatsapp):
        ...
        mock_llm.analyze_message.return_value = {
            "intent": "BOOK_APPOINTMENT",
            "extracted": {"barber_name": "Juan Perez", "date": "invalid-date"},  # Malformed
            "reply": "check",
        }
    ...
    def test_availability_time_period(self, mock_llm, mock_whatsapp):
        ...
        mock_llm.analyze_message.return_value = {
            "intent": "BOOK_APPOINTMENT",
            "extracted": {"barber_name": "Juan Perez", "date": "2023-12-01", "time_period": "tarde"},  # Afternoon
            "reply": "check",
        }
